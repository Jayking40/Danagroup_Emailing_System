...
async function seed() {
  try {
    ...
    if (!hqSubsidiary) {
      hqSubsidiary = await subRepo.save(subRepo.create({ name: 'Dana Group HQ', domain: 'danagroup.com' }));
    }

    let logisticsSubsidiary = await subRepo.findOneBy({ name: 'Dana Logistics' });
    if (!logisticsSubsidiary) {
      logisticsSubsidiary = await subRepo.save(subRepo.create({ name: 'Dana Logistics', domain: 'logistics.danagroup.com' }));
    }

    let manufacturingSubsidiary = await subRepo.findOneBy({ name: 'Dana Manufacturing' });
    if (!manufacturingSubsidiary) {
      manufacturingSubsidiary = await subRepo.save(subRepo.create({ name: 'Dana Manufacturing', domain: 'factory.danagroup.com' }));
    }

    // 2. Setup Departments (Check before creating)
    let itDept = await deptRepo.findOneBy({ name: 'IT', subsidiary_id: hqSubsidiary.id });
    if (!itDept) {
      itDept = await deptRepo.save(deptRepo.create({ name: 'IT', subsidiary_id: hqSubsidiary.id }));
    }

    let hrDept = await deptRepo.findOneBy({ name: 'HR', subsidiary_id: hqSubsidiary.id });
    if (!hrDept) {
      hrDept = await deptRepo.save(deptRepo.create({ name: 'HR', subsidiary_id: hqSubsidiary.id }));
    }
    ...
    if (!legalDept) {
      legalDept = await deptRepo.save(deptRepo.create({ 
        name: 'Legal', 
        subsidiary_id: hqSubsidiary.id 
      }));
    ...
    if (!financeDept) {
      financeDept = await deptRepo.save(deptRepo.create({ 
        name: 'Finance', 
        subsidiary_id: manufacturingSubsidiary.id 
      }));
    ...
    if (!opsDept) {
      opsDept = await deptRepo.save(deptRepo.create({ 
        name: 'Operations', 
        subsidiary_id: logisticsSubsidiary.id 
      }));
    }
