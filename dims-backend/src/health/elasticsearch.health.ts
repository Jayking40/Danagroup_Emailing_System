...
export class ElasticsearchHealthIndicator {
  constructor(private healthIndicatorService: HealthIndicatorService, private configService: ConfigService) {}
  ...
  async isHealthy(key: string) {
    ...
    try {
      ...
      const res = await axios.get(`${this.configService.get<string>('ELASTICSEARCH_NODE')}/_cluster/health`);
